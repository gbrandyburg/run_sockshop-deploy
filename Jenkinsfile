node {
    ...
    stage('Push image') {
        ...
        try {
            docker.withRegistry('https://nexusdocker-master-furyan.gbrandyburg.demo.twistlock.com', 'twistlock_creds') {
                app.push("0.3.12")
            }
        }catch(error) {
    ...
    stage('Deploy Sock-Shop') {
        withCredentials([usernamePassword(credentialsId: 'twistlock_creds', passwordVariable: 'TL_PASS', usernameVariable: 'TL_USER')]) {
            sh 'kubectl delete secret nexus-pull-secret -n sock-shop --ignore-not-found=true'
